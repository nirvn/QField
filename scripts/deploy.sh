...
if [[ ${TRAVIS_SECURE_ENV_VARS} = true ]]; then
  if [ ${TRAVIS_PULL_REQUEST} != false ]; then
    ...
    ./scripts/release-upload.py --release=${TRAVIS_TAG} --oauth-token=${GITHUB_API_TOKEN} /tmp/qfield-${TRAVIS_TAG}-armv7.apk /tmp/qfield-${TRAVIS_TAG}-x86.apk /tmp/qfield-${TRAVIS_TAG}-arm64_v8a.apk /tmp/qfield-${TRAVIS_TAG}-x86-Android-5.apk /tmp/qfield-${TRAVIS_TAG}-x86-Android-5.apk
